...
export function TagFilterContent({ availableTags, selectedTags, onTagToggle, onClearAll, onDone, autoFocus = true }: TagFilterContentProps) {
    ...
    const handleKeyDown = (e: KeyboardEvent<HTMLInputElement>) => {
        if (e.key === "Tab" && autocompleteSuggestion) {
            ...
            onTagToggle(selectedTags[selectedTags.length - 1]);
        }
    };
    ...
    return (
        <div className="space-y-3">
            ...
            <div className="text-xs flex flex-wrap gap-x-4 gap-y-1 pt-1" style={{ color: currentTheme.styles.contentTertiary }}>
                <span><kbd className="px-1 py-0.5 rounded text-caption" style={{ backgroundColor: currentTheme.styles.surfaceSecondary }}>Tab</kbd> autocomplete</span>
                <span><kbd className="px-1 py-0.5 rounded text-caption" style={{ backgroundColor: currentTheme.styles.surfaceSecondary }}>Enter</kbd> add filter</span>
                {onDone && (
                    <span><kbd className="px-1 py-0.5 rounded text-caption" style={{ backgroundColor: currentTheme.styles.surfaceSecondary }}>⌘ Enter</kbd> done</span>
                )}
            </div>
        </div>
    );
...
// ─── Original TagFilter with Dialog wrapper ─────────────────────────────────
