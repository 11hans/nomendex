...
export async function onStartup(): Promise<SkillUpdateCheckResult | null> {
    ...
    try {
        ...
        if (!(await gitignoreFile.exists())) {
            ...
            if (changed) {
                ...
                startupLogger.info("Updated .gitignore to allow projects.json");
            }
        }
    } catch (error) {
    ...
    startupLogger.info("Checking project migration status...");
    try {
        const { migrateProjects } = await import("./features/projects/projects-migration");
        await migrateProjects();
    } catch (error) {
        startupLogger.error("Failed to run project migration", {
            error: error instanceof Error ? error.message : String(error),
        });
    }
