...
class ReminderManager {
        ...
        eventStore.fetchReminders(matching: predicate) { [weak self] currentReminders in
            ...
            DispatchQueue.main.sync {
                for (taskId, oldState) in self.knownReminderStates {
                    if taskId == self.ignoreNextChangeForTaskID {
                        self.ignoreNextChangeForTaskID = nil
                        continue
                    }
                    ...
                    if let current = currentMap[taskId] {
                        ...
                        // We could compare due dates if we want, but typically you move dates less often in Reminders
                        // than in Calendar, and checking DateComponents equality is a bit tricky.
                        // Let's implement partial check if needed, but the main thing is `completed` state.
    ...
    private func upsertReminder(taskData: [String: Any], webView: WKWebView?, callback: String?) {
        ...
        let reminder = findReminder(taskId: taskId, in: list) ?? EKReminder(eventStore: eventStore)
        ...
        if let dateStr = targetDateStr, let parsedDate = parseISO(dateStr) {
            ...
            if let priority = taskData["priority"] as? String {
                if let alarm = alarmForPriority(priority) {
    ...
    private func deleteReminder(taskData: [String: Any], webView: WKWebView?, callback: String?) {
        ...
        if let reminder = findReminder(taskId: taskId, in: list) {
            // Prevent echo
            ignoreNextChangeForTaskID = taskId
            do {
    ...
    private func findReminder(taskId: String, in list: EKCalendar) -> EKReminder? {
        let predicate = eventStore.predicateForReminders(in: [list])
        ...
        // fetchReminders is asynchronous
        eventStore.fetchReminders(matching: predicate) { reminders in
            let targetURL = URL(string: "nomendex://task/\(taskId)")
            foundReminder = reminders?.first { $0.url == targetURL }
            group.leave()
        }
        
        // Since we are called from JS which is asynchronous, blocking here momentarily is okay,
        // but it's better to use semaphore or just wait. It's usually very fast.
